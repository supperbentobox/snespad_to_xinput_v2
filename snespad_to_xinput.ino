...
void loop() {
  ...
  // Buttons  swapeados para: snes -> xbox - > psx
  XInput.setButton(BUTTON_A, !buttons[SNES_BUTTON_B]);
  XInput.setButton(BUTTON_B, !buttons[SNES_BUTTON_A]);
  XInput.setButton(BUTTON_X, !buttons[SNES_BUTTON_Y]);
  XInput.setButton(BUTTON_Y, !buttons[SNES_BUTTON_X]);
  XInput.setButton(BUTTON_LB, !buttons[SNES_BUTTON_L]);
  XInput.setButton(BUTTON_RB, !buttons[SNES_BUTTON_R]);

  if (!buttons[SNES_BUTTON_ST] && !buttons[SNES_BUTTON_SEL] ) {
    XInput.setButton(BUTTON_LOGO, 1);
    XInput.setButton(BUTTON_START, 0);
    XInput.setButton(TRIGGER_RIGHT, 0);
  } else {
    XInput.setButton(BUTTON_LOGO, 0);
    XInput.setButton(BUTTON_START, !buttons[SNES_BUTTON_SEL]);
    //XInput.setButton(BUTTON_BACK,    !buttons[SNES_BUTTON_SEL]);
    XInput.setButton(TRIGGER_RIGHT, !buttons[SNES_BUTTON_ST]);
    //XInput.setTrigger(TRIGGER_RIGHT, !buttons[SNES_BUTTON_SEL]);
  }
